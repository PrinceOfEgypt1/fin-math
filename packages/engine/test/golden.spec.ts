...
// Importa direto do fonte do engine (evita depender de build/dist)
import * as engine from "../src/index";

// Comparação com tolerância absoluta
const approx = (a: number, b: number, tol: number) =>
  Math.abs(Number(a) - Number(b)) <= Number(tol);

// Caminho dos Golden Files (robusto)
const GF_DIR = path.resolve(__dirname, "../golden/starter");

// Considere só arquivos de teste com prefixos conhecidos (ignora README etc.)
...
// util para arredondar a 2 casas (financeiro)
  ...
  for (const fname of files) {
    ...
    it(`${id} — ${gf.description ?? ""}`.trim(), () => {
      if (id.startsWith("JC_")) {
        ...
        // Price — alinhar comparação a 2 casas e tolerância mais generosa
        const { inputs, expected } = gf;
        ...
        // SAC — compat de campo amortização constante
        const { inputs, expected } = gf;
